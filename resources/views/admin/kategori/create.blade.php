...
@section('title', 'Tambah Kategori SKPI')
@section('page-title', 'Tambah Kategori SKPI')
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('admin.kategori.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama" value="{{ old('nama') }}" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500">
                @error('nama')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
            </div>

            <div class="mb-4">
                <label class="block text-gray-700 font-semibold mb-2">Nama Kategori (English)</label>
                <input type="text" name="nama_en" value="{{ old('nama_en') }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="nilai" value="{{ old('nilai', 0) }}" min="0" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500">
                @error('nilai')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
            </div>

            <div class="mb-4">
                <label class="block text-gray-700 font-semibold mb-2">Deskripsi</label>
                <textarea name="deskripsi" rows="4" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500">{{ old('deskripsi') }}</textarea>
